...
awk '{gsub("metis.metis_lm_lss_wkf","METIS EDPS LSS LM-Band Workflow",$0); print $0}' tmp.dot > tmp1.dot
awk '{gsub("fontcolor=red","fontcolor=blue",$0); print $0}' tmp1.dot > tmp2.dot
dot -T png -Grankdir=TB tmp2.dot > metis_lm_lss_wkf_g2_vert.png
rm tmp.dot tmp1.dot

edps -w metis.metis_lm_lss_wkf -g > tmp3.dot
awk '{gsub("metis.metis_lm_lss_wkf","METIS EDPS LSS LM-Band Workflow",$0); print $0}' tmp3.dot > tmp4.dot
awk '{gsub("fontcolor=red","fontcolor=blue",$0); print $0}' tmp4.dot > tmp5.dot
dot -T png -Grankdir=LR tmp5.dot > metis_lm_lss_wkf_g1_vert.png
rm tmp*.dot
