...
dark_2rg_lm_lss_task = (task('metis_lm_lss_dark')
...
linearity_lm_lss_task = (task('metis_lm_lss_lingain')
            .with_recipe("metis_det_lingain")
            .with_main_input(detlin_2rg_raw)
            .with_associated_input(dark_2rg_lm_lss_task, min_ret=0) # min_ret=0 --> optional input            # .with_main_input(raw_dark)       # check what the minimum number of raw darks is!
            # .with_associated_input(static_persistence_map, min_ret=0) # min_ret=0 --> optional input
            .with_associated_input(lm_wcu_off_raw)        # check how that is exactly delivered by the ICS!
            .with_meta_targets([QC1_CALIB])
            .build())

flatfield_lm_lss_task = (task('metis_lm_lss_rsrf')
            .with_recipe("metis_lm_lss_rsrf")
            .with_main_input(lm_raw_rsrf)
            .with_associated_input(lm_wcu_off_raw)
            .with_associated_input(dark_2rg_lm_lss_task)
            .with_associated_input(linearity_lm_lss_task)
            .with_meta_targets([QC1_CALIB])
            .build())

trace_finding_lm_lss_task = (task("metis_lm_lss_trace")
            .with_recipe("metis_lm_lss_trace")
            .with_main_input(lm_raw_rsrf_pinh)
            .with_associated_input(lm_wcu_off_raw)
            .with_associated_input(dark_2rg_lm_lss_task)
            .with_associated_input(linearity_lm_lss_task)
            .with_associated_input(flatfield_lm_lss_task)
            .with_associated_input(calib_persistence, min_ret=0)
            .with_meta_targets([QC1_CALIB])
            .build())

wavelength_calibration_lm_lss_task = (task("metis_lm_lss_wave")
            ...
            .with_associated_input(linearity_lm_lss_task)
            .with_associated_input(flatfield_lm_lss_task)
            .with_associated_input(trace_finding_lm_lss_task)
            .with_meta_targets([QC1_CALIB])
            .build())

std_reduction_lm_lss_task = (task('metis_lm_lss_std')
            ...
            .with_associated_input(flatfield_lm_lss_task)
            .with_associated_input(trace_finding_lm_lss_task)
            .with_associated_input(wavelength_calibration_lm_lss_task)
            .with_associated_input(slitloss_lm_lss_task)
...
sci_reduction_lm_lss_task = (task('metis_lm_lss_sci')
            ...
            .with_associated_input(static_lm_lss_dist_sol)
            .with_associated_input(static_lm_lss_wave_guess)
            .with_associated_input(std_reduction_lm_lss_task)
            .with_associated_input(static_atm_line_cat)
            .with_associated_input(static_lm_adc_slitloss)
            .with_associated_input(static_ao_psf_model)
...
mf_model_lm_lss_task = (task("metis_lm_lss_mf_model")
            .with_main_input(sci_reduction_lm_lss_task)
            .with_associated_input(std_reduction_lm_lss_task)
            # .with_associated_input(static_ao_psf_model)
            .with_associated_input(static_atm_line_cat)
            ...
            .with_associated_input(sci_reduction_lm_lss_task, condition=on_science)
            .with_associated_input(std_reduction_lm_lss_task, condition=on_standard)
            .with_recipe('metis_lm_lss_mf_model')
            .with_meta_targets([QC1_CALIB])
            .build())

mf_calctrans_lm_lss_task = (task("metis_lm_lss_mf_calctrans")
            .with_main_input(mf_model_lm_lss_task)
            .with_associated_input(static_atm_line_cat)
            .with_associated_input(calib_lsf_kernel)
            .with_associated_input(calib_atm_profile)
...
mf_correct_lm_lss_task = (task("metis_lm_lss_mf_correct")
            .with_main_input(sci_reduction_lm_lss_task)
            .with_associated_input(mf_calctrans_lm_lss_task)
