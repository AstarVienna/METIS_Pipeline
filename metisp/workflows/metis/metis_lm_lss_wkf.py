...
lm_lss_dark_task = (task('metis_lm_lss_dark')
            ...
            .with_associated_input(lm_lss_lingain_task, [linearity_h2rg_class, gain_map_h2rg_class])
...
lm_lss_adc_slitloss_task = (task('metis_lm_lss_adc_slitloss')
            ...
            .with_associated_input(persistence_map)
            .with_associated_input(lm_lss_lingain_task, [linearity_h2rg_class, gain_map_h2rg_class])
            .with_associated_input(lm_lss_dark_task, [master_dark_2rg_class])
            .with_meta_targets([QC1_CALIB])
            .build())
...
lm_lss_trace_task = (task("metis_lm_lss_trace")
            ...
            .with_associated_input(lm_lss_dark_task, [master_dark_2rg_class])
            .with_associated_input(lm_lss_lingain_task, [linearity_h2rg_class, gain_map_h2rg_class])
            .with_associated_input(lm_lss_rsrf_task, [master_lm_lss_rsrf_class])
            .with_associated_input(persistence_map)
            .with_meta_targets([QC1_CALIB])
            .build())
...
lm_lss_wave_task = (task("metis_lm_lss_wave")
            ...
            .with_associated_input(lm_lss_lingain_task, [linearity_h2rg_class, gain_map_h2rg_class])
            .with_associated_input(lm_lss_rsrf_task, [master_lm_lss_rsrf_class])
            .with_associated_input(lm_lss_trace_task, [lm_lss_trace_class])
            .with_meta_targets([QC1_CALIB])
            .build())
...
lm_lss_std_task = (task('metis_lm_lss_std')
            ...
            .with_associated_input(lm_lss_rsrf_task, [master_lm_lss_rsrf_class])
            .with_associated_input(lm_lss_trace_task, [lm_lss_trace_class])
            .with_associated_input(lm_lss_wave_task, [lm_lss_dist_sol_class, lm_lss_wave_guess_class])
            .with_associated_input(lm_lss_adc_slitloss_task, [lm_adc_slitloss_class])
            .with_associated_input(atm_line_cat)
            .with_associated_input(ref_std_cat)
            .with_associated_input(lm_synth_trans)
            .with_associated_input(lm_lss_dist_sol)
            .with_associated_input(lm_lss_wave_guess)
            .with_associated_input(lm_lss_adc_slitloss_task, [lm_adc_slitloss_class]) # Changed to the task for now since the calibration Product doesn't exist in Simulations yet
...
lm_lss_sci_task = (task('metis_lm_lss_sci')
            ...
            .with_associated_input(lm_lss_dist_sol)
            .with_associated_input(lm_lss_wave_guess)
            .with_associated_input(lm_lss_std_task, [master_lm_response_class, std_transmission_class])
            .with_associated_input(atm_line_cat)
            .with_associated_input(lm_lss_adc_slitloss_task, [lm_adc_slitloss_class]) # Changed to the task for now since the calibration Product doesn't exist in Simulations yet
            .with_associated_input(ao_psf_model)
...
lm_lss_mf_model_task = (task("metis_lm_lss_mf_model")
            .with_main_input(lm_lss_sci_task, [lm_lss_sci_flux_1d_class])
            ...
            .with_associated_input(lm_lss_sci_task, [lm_lss_sci_flux_1d_class], condition=on_science)
            .with_associated_input(lm_lss_std_task, [lm_lss_std_1d_class], condition=on_standard)
            .with_recipe('metis_lm_lss_mf_model')
            .with_meta_targets([QC1_CALIB])
            .build())

lm_lss_mf_calctrans_task = (task("metis_lm_lss_mf_calctrans")
            .with_main_input(lm_lss_mf_model_task, [mf_best_fit_tab_class])
            .with_associated_input(atm_line_cat)
            .with_associated_input(lsf_kernel)
            .with_associated_input(atm_profile)
...
lm_lss_mf_correct_task = (task("metis_lm_lss_mf_correct")
            .with_main_input(lm_lss_sci_task, [lm_lss_sci_flux_1d_class])
            .with_associated_input(lm_lss_mf_calctrans_task, [lm_lss_synth_trans_class])
