...
n_lss_dark_task = (task('metis_n_lss_dark')
            ...
            .with_associated_input(n_lss_lingain_task, [linearity_geo_class, gain_map_geo_class])
            .with_meta_targets([QC1_CALIB])
            .build())
...
n_adc_slitloss_task = (task('metis_n_adc_slitloss')
            ...
            .with_associated_input(n_wcu_off_raw)        # check how that is exactly delivered by the ICS!
            .with_associated_input(persistence_map)
            .with_associated_input(n_lss_lingain_task, [linearity_geo_class, gain_map_geo_class]) # Shouldn't this also need a MASTER_IMG_FLAT_LAMP_N as per the DRLD?
            .with_associated_input(n_lss_dark_task, [master_dark_geo_class])
            .with_meta_targets([QC1_CALIB])
            .build())
...
n_lss_trace_task = (task("metis_n_lss_trace")
            ...
            .with_associated_input(n_lss_dark_task, [master_dark_geo_class])
            .with_associated_input(n_lss_lingain_task, [linearity_geo_class, gain_map_geo_class])
            .with_associated_input(n_lss_rsrf_task, [master_n_lss_rsrf_class])
            .with_associated_input(persistence_map)
            .with_meta_targets([QC1_CALIB])
            .build())
...
n_lss_std_task = (task('metis_n_lss_std')
            ...
            .with_associated_input(n_lss_dark_task, [master_dark_geo_class])
            .with_associated_input(n_lss_rsrf_task, [master_n_lss_rsrf_class])
            .with_associated_input(n_lss_trace_task, [n_lss_trace_class])
            # .with_associated_input(wavelength_calibration_task)
            .with_associated_input(n_adc_slitloss_task, [n_adc_slitloss_class])
            .with_associated_input(atm_line_cat)
            .with_associated_input(ref_std_cat)
            .with_associated_input(n_synth_trans)
            .with_associated_input(n_lss_dist_sol)
            .with_associated_input(n_lss_wave_guess)
            .with_associated_input(n_adc_slitloss_task, [n_adc_slitloss_class]) # Changed to the task for now since the calibration Product doesn't exist in Simulations yet
...
n_lss_sci_task = (task('metis_n_lss_sci')
            ...
            .with_associated_input(n_lss_dist_sol)
            .with_associated_input(n_lss_wave_guess)
            .with_associated_input(n_lss_std_task, [master_n_response_class, std_transmission_class])
            .with_associated_input(atm_line_cat)
            .with_associated_input(n_adc_slitloss_task, [n_adc_slitloss_class]) # Changed to the task for now since the calibration Product doesn't exist in Simulations yet
            .with_associated_input(ao_psf_model)
...
n_lss_mf_model_task = (task("metis_n_lss_mf_model")
            .with_main_input(n_lss_std_task, [n_lss_std_1d_class])
            ...
            .with_associated_input(n_lss_std_task, [n_lss_std_1d_class], condition=on_standard)
            .with_associated_input(n_lss_sci_task, [n_lss_sci_flux_1d_class], condition=on_science)
            .with_recipe('metis_n_lss_mf_model')
            .with_meta_targets([QC1_CALIB])
            .build())

n_lss_mf_calctrans_task = (task("metis_n_lss_mf_calctrans")
            .with_main_input(n_lss_mf_model_task, [mf_best_fit_tab_class])
            .with_associated_input(atm_line_cat)
            .with_associated_input(lsf_kernel)
            .with_associated_input(atm_profile)
...
n_lss_mf_correct_task = (task("metis_n_lss_mf_correct")
            .with_main_input(n_lss_sci_task, [n_lss_sci_flux_1d_class])
            .with_associated_input(n_lss_mf_calctrans_task, [n_lss_synth_trans_class])
