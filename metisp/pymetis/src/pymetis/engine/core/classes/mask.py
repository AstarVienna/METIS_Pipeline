...
class Mask:
    """
    A 32-bit data-quality mask: every pixel holds a bitfield whose individual
    bits flag independent defect classes. The meaning of each bit is declared
    by the instrument (see ``Metis.MaskFlags``); this class stays numeric.
    ...
    :meth:`flatten` (see DRLD section 3.5.3).
    ...
    ``CplType.INT`` is a *signed* int32, so bit 31 is the sign bit -- only
    ...
    def __and__(self, other: Self) -> Self:
        ...
        return Mask(CplImage(self._array() & other._array(), dtype=CplType.INT))
