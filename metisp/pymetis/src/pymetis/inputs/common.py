...
# Static calibration files required for LSS mode ++++++++++++++++++++++++
# Table with laser wavelengthes
class LaserTableInput(SinglePipelineInput):
    _title: str = "laser line table"
    _tags: Pattern = re.compile(r"LASER_TAB")
    _group: cpl.ui.Frame.FrameGroup = cpl.ui.Frame.FrameGroup.CALIB

# Standard star catalogue for FLux calibration / telluric correction
class ReferenceStandardCatalogueInput(SinglePipelineInput):
    _title: str = "Catalogue of reference stars"
    _tags: Pattern = re.compile(r"REF_STD_CAT")
    _group: cpl.ui.Frame.FrameGroup = cpl.ui.Frame.FrameGroup.CALIB

# Synthetic transmission curves used for the flux calibration
class SynthTransSpectrumInput(SinglePipelineInput):
    _title: str = "Transmission for flux calibration"
    _tags: Pattern = re.compile(r"(?P<detector>LM|N)_SYNTH_TRANS")
    _group: cpl.ui.Frame.FrameGroup = cpl.ui.Frame.FrameGroup.CALIB

# Atmospheric line database
class AtmLineCatInput(SinglePipelineInput):
    _title: str = "Atmospheric line table"
    _tags: Pattern = re.compile(r"ATM_LINE_CAT")
    _group: cpl.ui.Frame.FrameGroup = cpl.ui.Frame.FrameGroup.CALIB

# Model of the AO PSF TODO: Define details
class AOPSFModelInput(SinglePipelineInput):
    _title: str = "AO PSF Model"
    _tags: Pattern = re.compile(r"AP_PSF_MODEL")
    _group: cpl.ui.Frame.FrameGroup = cpl.ui.Frame.FrameGroup.CALIB

# N-band first guess distortion correction table
class NLSSDistSolTableInput(SinglePipelineInput):
    _title: str = "First guess distortion solution for the N-band LSS mode"
    _tags: Pattern = re.compile(r"N_LSS_DIST_COL")
    _group: cpl.ui.Frame.FrameGroup = cpl.ui.Frame.FrameGroup.CALIB

# N-band first guess wavelength solution
class NLSSWaveGuessInput(SinglePipelineInput):
    _title: str = "First guess LSS N-and wavelength solution"
    _tags: Pattern = re.compile(r"N_LSS_WAVE_GUESS")
    _group: cpl.ui.Frame.FrameGroup = cpl.ui.Frame.FrameGroup.CALIB

# TODO: Add LM/N_ADC_SLITLOSS input
